  ...
  const LoginPortal = () => {
    ...
    const handleLogin = async (e) => {
      e.preventDefault();
      const apiUrl = 'https://partner-api.21express.co.id/users/account/login';
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-blue-400 to-blue-800">
        <form onSubmit={handleLogin} className="bg-white p-8 rounded-lg shadow-lg max-w-sm w-full">
          ...
          <input
            ...
            required
            className="border-2 border-gray-300 p-3 rounded-lg mb-6 w-f ull focus:outline-none focus:border-blue-500 transition"
          />
          <button
            type="submit"
